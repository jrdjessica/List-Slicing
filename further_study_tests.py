...
class FurtherStudyTests(unittest.TestCase):
    ...
    def test_custom_len(self):
        result = further_study.custom_len(
    ...
    def tests_custom_extend(self):
        months = ['Jan', 'Feb', 'Mar']
        further_study.custom_extend(months, ['Apr', 'May'])
        self.asertEqual(months, ['Jan', 'Feb', 'Mar', 'Apr', 'May'])
    ...
    def test_custom_index(self):
        result = further_study.custom_index(
            ['Do', 'Re', 'Mi', 'Fa', 'So', 'La', 'Ti', 'Do'], 'Re')
        self.assertEqual(result, 1)

    def test_custom_count(self):
        result = further_study.custom_count(
            ['Do', 'Re', 'Mi', 'Fa', 'So', 'La', 'Ti', 'Do'], 'Do')
        self.assertEqual(result, 2)

    def test_custom_reverse(self):
        multiples = [0, 3, 6, 9, 12, 15, 18, 21, 24, 27]
        further_study.custom_reverse(multiples)
        self.assertEqual(multiples, [27, 24, 21, 18, 15, 12, 9, 6, 3, 0])

    def test_custom_contains(self):
        result = further_study.custom_contains(
            [0, 3, 6, 9, 12, 15, 18, 21, 24], 23)
        self.assertEqual(result, False)

        result2 = further_study.custom_contains(
            [0, 3, 6, 9, 12, 15, 18, 21, 24], 24)
        self.assertEqual(result2, True)

    def test_custom_equality(self):
        result = further_study.custom_equality(
            ['Jan', 'Feb', 'Mar'], ['Jan', 'Feb', 'Mar'])
        self.assertEqual(result, True)

        result2 = further_study.custom_equality(
            ['Jan', 'Feb', 'Mar'], ['Jan', 'Mar', 'Feb'])
